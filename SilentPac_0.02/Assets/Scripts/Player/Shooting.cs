...
public class Shooting : MonoBehaviour
{
    ...
    public void StartShooting()
    {
        ...
        Vector3 fwd = transformPointA.transform.forward;
        RaycastHit hit;

        if (Physics.Raycast(transformPointA.localPosition, transformPointA.transform.forward, out hit, length))
        {
            transformPointB.localPosition = transformPointA.TransformDirection(Vector3.forward) * hit.distance;
            Debug.DrawRay(transformPointA.position, transformPointA.TransformDirection(Vector3.forward) * hit.distance, Color.yellow);
        }
        else
        {
